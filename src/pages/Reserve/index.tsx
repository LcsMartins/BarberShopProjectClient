...
function getBusyHours(
  ...
  if (newAppointment.dateTime !== "") {
    ...
    appointments.map((item) =>
      item.dateTime.includes(aux[0]) &&
      item.barberId === newAppointment.barberId
        ? busyHours.push(item.dateTime)
        : null
    );
  }
  return busyHours;
...
const Reserve: React.FC = () => {
  ...
  const [appointmentsLoaded, setAppointmentsNew] =
  ...
  const loadReserves = useCallback(async (barberId: string) => {
    if (barberId !== "") {
      ...
      await fetch(`${api}appointments/${barberId}?flag=0`, {
        ...requestOptions,
      })
        .then((response) => response.json())
  ...
  useEffect(() => {
    loadReserves(newAppointment.barberId);
  }, [loadReserves, newAppointment.barberId]);
  ...
  useEffect(() => {
    setOcupadas(getBusyHours(appointmentsLoaded, newAppointment));
  }, [appointmentsLoaded, newAppointment]);

  const [disponiveis, setDisponiveis] = useState([""]);
  useEffect(() => {
    setDisponiveis(getAvailableHours(ocupadas));
  }, [ocupadas]);
  ...
  const handleBarberChange = (event: any) => {
    ...
    setUserFeedback("");
    setNewAppointment((prevAppointments) => ({
      ...prevAppointments,
  ...
  const handleHourChange = (event: any) => {
    ...
    if (selectedHour !== "") {
      ...
      setNewAppointment((prevAppointments) => ({
        ...prevAppointments,
        dateTime: oldDate[0] + "T" + selectedHour,
      }));
      setNewAppointment((prevAppointments) => ({
        ...prevAppointments,
        customerId: id,
  ...
  const handleSubmit = () => {
    ...
    fetch(`${api}appointment`, { ...requestOptions }).then((response) =>
  ...
  useEffect(() => {
    console.log(newAppointment, selectedHourFlag);
  }, [newAppointment, selectedHourFlag]);
  return (
    <div>
      <MainContainer>
        <SugestaoSection>
          ...
          <Section
            name="day"
            required
            disabled={newAppointment.barberId === "" ? true : false}
            onChange={handleDayChange}
          >
            <option value="" selected={selectedDayFlag}>
              Selecione o dia
            </option>
            ...
            {currentDate.numberDaysNextMonth.map((d) => (
              <option
                key={d}
                value={[String(d), String(currentDate.month + 1)]}
              >
          ...
          <Section
            ...
            required
            disabled={
              newAppointment.barberId === "" || newAppointment.dateTime === ""
                ? true
                : false
            }
            onChange={handleHourChange}
          >
            <option value="" selected={selectedHourFlag}>
              Selecione o horário
            </option>
            {disponiveis.map((h) => (
        ...
        {userFeedback !== "" ? (
          <h3>
            Horário marcado no dia {printDay(newAppointment.dateTime)}, às{" "}
            {newAppointment.dateTime.split("T")[1]} horas, com o barbeiro{" "}
            {newAppointment.barberId} com sucesso!
          </h3>
        ) : null}
      </MainContainer>
    </div>
